...
namespace IdentityServer3.AccessTokenValidation
{
    internal class IntrospectionEndpointTokenProvider : AuthenticationTokenProvider
    {
        ...
        {
            _logger = loggerFactory.Create("IntrospectionEndpointTokenProvider");
        ...
        public override async Task ReceiveAsync(AuthenticationTokenReceiveContext context)
        {
            ...
            {
                { "token", context.Token },
                { "client_id", _options.ScopeId },
                { "client_secret", _options.ScopeSecret }
            };
            ...
            try
            {
                ...
                {
                    _logger.WriteInformation("Error returned from introspection endpoint: " + response.ReasonPhrase);
                    return;
                }
            }
            catch (Exception ex)
            ...
            object activeClaim;
            if (dictionary.TryGetValue("active", out activeClaim))
            {
                bool active;
                if (bool.TryParse(activeClaim.ToString(), out active))
                {
                    if (active == false)
                    {
                        _logger.WriteInformation("Token is not active anymore.");
                        return;
                    }
                }
                else
                {
                    _logger.WriteError("malformed active claim.");
                    return;
                }
            }
            else
            {
                _logger.WriteError("No active claim in response found");
                return;
            }
